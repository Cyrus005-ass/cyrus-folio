...
<section class='admin-grid'>
    <div class='panel'>
        ...
        <form class='form' method='post' action='<?= url('/admin/skills') ?>'>
            ...
            <div class='form-row'>
                <label><span class='label'>Categorie</span><select class='select' name='categorie'><?php foreach (skill_category_options() as $category): ?><option value='<?= e($category) ?>'><?= e($category) ?></option><?php endforeach; ?></select></label>
                <label><span class='label'>Niveau</span><select class='select' name='niveau'><?php foreach (skill_level_options() as $level): ?><option value='<?= e($level) ?>' <?= $level === 'Intermediaire' ? 'selected' : '' ?>><?= e($level) ?></option><?php endforeach; ?></select></label>
            </div>
            <label><span class='label'>Description</span><textarea class='textarea' name='description'></textarea></label>
            <div class='form-row'>
                <label><span class='label'>Icone</span><input class='input' type='text' name='icone'></label>
    ...
    <div class='panel'>
                            ...
                            <?php foreach ($items as $skill): ?>
                                <form class='form card' method='post' action='<?= url('/admin/skills/' . $skill['id']) ?>' style='padding:16px;'>
                                    ...
                                    <div class='form-row'>
                                        ...
                                        <label><span class='label'>Categorie</span><select class='select' name='categorie'><?php foreach (skill_category_options() as $option): ?><option value='<?= e($option) ?>' <?= ($skill['categorie'] ?? '') === $option ? 'selected' : '' ?>><?= e($option) ?></option><?php endforeach; ?></select></label>
                                    </div>
                                    <div class='form-row'>
                                        <label><span class='label'>Niveau</span><select class='select' name='niveau'><?php foreach (skill_level_options() as $level): ?><option value='<?= e($level) ?>' <?= ($skill['niveau'] ?? '') === $level ? 'selected' : '' ?>><?= e($level) ?></option><?php endforeach; ?></select></label>
                                    ...
                                    <div class='form-row'>
                                        <label><span class='label'>Icone</span><input class='input' type='text' name='icone' value='<?= e($skill['icone'] ?? '') ?>'></label>
                                ...
                                <form method='post' action='<?= url('/admin/skills/' . $skill['id']) ?>'>
                                    <?= csrf_field() ?>
                                    <?= method_field('DELETE') ?>
                                    <button class='btn danger' type='submit' data-confirm='Supprimer cette competence ?'>Supprimer</button>
                                </form>
                            <?php endforeach; ?>
                        </div>
                    </div>
        ...
        <?php else: ?>
            <div class='empty'>Aucune competence enregistree.</div>
        <?php endif; ?>
    </div>
</section>
